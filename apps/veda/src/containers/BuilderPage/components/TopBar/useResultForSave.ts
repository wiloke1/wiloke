...
const useResultForSave = () => {
  ...
  const getResultOfPages = async (errorOption: ErrorOption): Promise<Result['pages']> => {
    const resultInArray = await Promise.all(
      Object.entries(pages.data).map(async ([pageId, page]) => {
        ...
        const pageScreenshot = await getScreenshot(pageId);
  ...
  const getResult = async (errorOption: ErrorOption): Promise<Result> => {
    ...
    try {
      const result: Result = {
        ...
        pages: await getResultOfPages(errorOption),
