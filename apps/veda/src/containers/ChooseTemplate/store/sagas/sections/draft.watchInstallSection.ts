...
import { installDraftSection, setTemplateBoardVisible } from '../../actions';

function* handleAdd({ payload: { commandId } }: ReturnType<typeof installDraftSection.request>) {
  try {
    ...
    const response: Awaited<ReturnType<typeof sectionService.sections.installDraftSection>> = yield retry(
      3,
      1000,
      sectionService.sections.installDraftSection,
      commandId,
    );

    if (response.megaMenuCommandIds.length > 0) {
      const responseMegaMenu: Awaited<ReturnType<typeof sectionService.megaMenus.searchMegaMenusOfDraft>> = yield retry(
        3,
        1000,
    ...
    yield put(setTemplateBoardVisible({ visible: false, navKeys: ['draft', 'Sections'] }));
    yield put(installDraftSection.success({ commandId }));

    yield delay(100);
    setScrollTo(`[data-id="${_section.id}"]`, { timeout: 100 });
  } catch (error) {
    yield put(installDraftSection.failure({ commandId }));
  }
}

export function* watchInstallDraftSection() {
  yield takeLatest(getActionType(installDraftSection.request), handleAdd);
}
