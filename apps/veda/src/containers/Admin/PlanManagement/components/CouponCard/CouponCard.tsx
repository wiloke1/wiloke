...
export const CouponCard: React.FC<CouponCardProps> = ({ code, discountText, expireOn, deleteLoading = false, onEdit, onDelete }) => {
  return (
    <View css={styles.container}>
      <View css={styles.body} onClick={onEdit}>
        <View>
          <View css={styles.discount}>{discountText}</View>
          <View css={styles.code}>{code}</View>
          <Space size={4} />
          <View css={styles.expiredOn}>{expireOn}</View>
        </View>
        <View css={styles.logo}>
      ...
      <Confirm
        ...
        title={`Delete`}
        message={`Confirm delete ${code}`}
      >
        <Button size="extra-small" radius={4} backgroundColor="danger" color="light" loading={deleteLoading}>
          Delete
        </Button>
      </Confirm>
    </View>
