...
import { actionUpdateStatusHomePage } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusHomePage.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusHomePage.success({
      ...
      yield put(actionUpdateStatusHomePage.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusHomePage.failure({ id: payload.id }));
...
export function* watchUpdateStatusHomePage() {
  yield takeEvery(getActionType(actionUpdateStatusHomePage.request), handle);
}
