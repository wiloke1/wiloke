...
import { actionLoadMoreHomePage } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreHomePage.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreHomePage.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreHomePage.failure(undefined));
  }
}

export function* watchLoadMoreHomePages() {
  yield takeLatest(getActionType(actionLoadMoreHomePage.request), handleGet);
}
