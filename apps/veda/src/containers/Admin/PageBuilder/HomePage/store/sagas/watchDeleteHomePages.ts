...
import { actionDeleteHomePages } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'home', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteHomePages.success({ ids: [id] }));
    yield put(syncToShopify.success(undefined));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteHomePages.failure({ id }));
  }
}

function* handleCreate({ payload }: ReturnType<typeof actionDeleteHomePages.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteHome: false }));
...
export function* watchDeleteHomePages() {
  yield takeLatest(getActionType(actionDeleteHomePages.request), handleCreate);
}
