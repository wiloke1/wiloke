...
import { actionUpdateStatusNotFoundPage } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusNotFoundPage.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusNotFoundPage.success({
      ...
      yield put(actionUpdateStatusNotFoundPage.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusNotFoundPage.failure({ id: payload.id }));
...
export function* watchUpdateStatusNotFoundPage() {
  yield takeEvery(getActionType(actionUpdateStatusNotFoundPage.request), handle);
}
