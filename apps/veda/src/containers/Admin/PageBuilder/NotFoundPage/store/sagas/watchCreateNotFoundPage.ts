...
import { actionCreateNotFoundPage } from '../actions';

function* handleCreate({ payload }: ReturnType<typeof actionCreateNotFoundPage.request>) {
  const { templateId, name, includeHeaderFooter, callback } = payload;
  try {
    const { isCreate }: ModalAdminState = yield select(modalsSelector);

    if (isCreate) {
      yield put(actionCreateNotFoundPage.success(undefined));
    ...
    yield put(actionCreateNotFoundPage.failure(undefined));
  }
}

export function* watchCreateNotFoundPage() {
  yield takeLatest(getActionType(actionCreateNotFoundPage.request), handleCreate);
}
