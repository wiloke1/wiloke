...
import { actionDeleteNotFoundPages } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'pageNotFound', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteNotFoundPages.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteNotFoundPages.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteNotFoundPages.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteNotFound: false }));
...
export function* watchDeleteNotFoundPages() {
  yield takeLatest(getActionType(actionDeleteNotFoundPages.request), handleDelete);
}
