...
import { authSelector, productPageSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { FontAwesome, View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteProductPages,
  useGetProductPageItems,
  useLoadMoreProductPage,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusProductPage,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(productPageSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusProductPage();
  ...
  const loadMoreRequest = useLoadMoreProductPage();
  const deletePageRequest = useDeleteProductPages();
  ...
  const handleOpenModalProduct = (item: AdminPageData) => () => {
    changeSettings({
      visibleListProduct: true,
      originPage: item,
      page: item,
      ...
      <View>
        {data.map(item => (
          <StatisticBox
            ...
            Right={
              <Tooltip text={'Select product'}>
                <View borderColor="gray3" borderStyle="solid" borderWidth={1} css={styles.rightItem} onClick={handleOpenModalProduct(item)}>
