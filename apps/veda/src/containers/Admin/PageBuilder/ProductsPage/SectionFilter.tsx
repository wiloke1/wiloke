...
export const SectionFilter = () => {
  const { ids, data } = useSelector(productPageSelector);

  const setSelectAll = useIsSelectAll();
  const setIds = useSelectManyItems();

  const handleSelectAll: CheckboxProps['onValueChange'] = isSelect => {
  ...
  return (
    <View row css={styles.sectionTableHeader}>
      <View css={styles.checkbox}>
        <Checkbox onValueChange={handleSelectAll} checked={ids.length > 0 && data.length > 0 && ids.length === data.length} />
      </View>
