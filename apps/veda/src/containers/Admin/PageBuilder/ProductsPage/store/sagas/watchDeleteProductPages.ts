...
import { actionDeleteProductPages } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'product', eventId }),
  });
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteProductPages.success({ ids: [id] }));
    yield put(syncToShopify.success(undefined));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteProductPages.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteProductPages.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteProduct: false }));
...
export function* watchDeleteProductPages() {
  yield takeLatest(getActionType(actionDeleteProductPages.request), handleDelete);
}
