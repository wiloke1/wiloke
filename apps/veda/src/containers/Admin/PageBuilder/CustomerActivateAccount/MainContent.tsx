...
import { authSelector, customerActivateAccountSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerActivateAccounts,
  useLoadMoreCustomerActivateAccount,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerActivateAccount,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerActivateAccountSelector);

  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerActivateAccount();
  ...
  const loadMoreRequest = useLoadMoreCustomerActivateAccount();
  const deletePageRequest = useDeleteCustomerActivateAccounts();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-activate-account',
