...
import { actionDeleteCustomerActivateAccounts } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'activateAccount', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerActivateAccounts.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerActivateAccounts.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerActivateAccounts.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersActivateAccount: false }));
...
export function* watchDeleteCustomerActivateAccount() {
  yield takeLatest(getActionType(actionDeleteCustomerActivateAccounts.request), handleDelete);
}
