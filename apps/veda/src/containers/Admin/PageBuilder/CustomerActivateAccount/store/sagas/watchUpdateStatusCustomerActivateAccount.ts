...
import { actionUpdateStatusCustomerActivateAccount } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCustomerActivateAccount.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCustomerActivateAccount.success({
      ...
      yield put(actionUpdateStatusCustomerActivateAccount.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCustomerActivateAccount.failure({ id: payload.id }));
...
export function* watchUpdateStatusCustomerActivateAccount() {
  yield takeEvery(getActionType(actionUpdateStatusCustomerActivateAccount.request), handle);
}
