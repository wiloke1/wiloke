...
import { actionUpdateStatusCartPage } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCartPage.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCartPage.success({
      ...
      yield put(actionUpdateStatusCartPage.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCartPage.failure({ id: payload.id }));
...
export function* watchUpdateStatusCartPage() {
  yield takeEvery(getActionType(actionUpdateStatusCartPage.request), handle);
}
