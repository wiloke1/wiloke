...
import { actionCreateCartPage } from '../actions';

function* handleCreate({ payload }: ReturnType<typeof actionCreateCartPage.request>) {
  const { templateId, name, includeHeaderFooter, callback } = payload;
  try {
    const { isCreate }: ModalAdminState = yield select(modalsSelector);

    if (isCreate) {
      yield put(actionCreateCartPage.success(undefined));
    ...
    yield put(actionCreateCartPage.failure(undefined));
  }
}

export function* watchCreateCartPage() {
  yield takeLatest(getActionType(actionCreateCartPage.request), handleCreate);
}
