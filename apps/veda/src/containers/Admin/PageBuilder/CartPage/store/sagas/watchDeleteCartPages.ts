...
import { actionDeleteCartPages } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'cart', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCartPages.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCartPages.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCartPages.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCart: false }));
...
export function* watchDeleteCartPages() {
  yield takeLatest(getActionType(actionDeleteCartPages.request), handleDelete);
}
