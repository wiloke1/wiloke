...
import { actionLoadMoreCartPage } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreCartPage.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreCartPage.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreCartPage.failure(undefined));
  }
}

export function* watchLoadMoreCartPages() {
  yield takeLatest(getActionType(actionLoadMoreCartPage.request), handleGet);
}
