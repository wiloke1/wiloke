...
import { authSelector, cartPageSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import { useChangeSearchKey, useDeleteCartPages, useLoadMoreCartPage, useSelectManyItems, useSetFilterType, useUpdateStatusCartPage } from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(cartPageSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCartPage();
  ...
  const loadMoreRequest = useLoadMoreCartPage();
        ...
        {data.map(item => (
          <StatisticBox
            ...
            onDelete={() => {
              connect({
                onSuccess: () => {
                  deleteCartRequest.request({ ids: [item.commandId], onFulfill: () => disconnect({}) });
                },
                onError: () => {
                  notification.error({
                    message: i18n.t('publish_shopify.init_sync_error'),
