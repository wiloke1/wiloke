...
import { authSelector, passwordPageSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeletePasswordPages,
  useLoadMorePasswordPage,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusPasswordPage,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(passwordPageSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusPasswordPage();
  ...
  const loadMoreRequest = useLoadMorePasswordPage();
  const deletePageRequest = useDeletePasswordPages();
  ...
  return (
    <View>
      ...
      <Box radius={6} borderColor="gray2" css={{ padding: '20px 10px' }}>
        <SectionFilter />
        <AsyncComponent
