...
import { actionDeletePasswordPages } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'password', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeletePasswordPages.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeletePasswordPages.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeletePasswordPages.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deletePassword: false }));
...
export function* watchDeletePasswordPages() {
  yield takeLatest(getActionType(actionDeletePasswordPages.request), handleDelete);
}
