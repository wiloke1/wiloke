...
import { actionUpdateStatusPasswordPage } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusPasswordPage.request>) {
  const { onFulfill } = payload;
  try {
    const { id, enable, adminPageData } = payload;
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusPasswordPage.success({
      ...
      yield put(actionUpdateStatusPasswordPage.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusPasswordPage.failure({ id: payload.id }));
...
export function* watchUpdateStatusPasswordPage() {
  yield takeEvery(getActionType(actionUpdateStatusPasswordPage.request), handle);
}
