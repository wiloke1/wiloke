...
import { actionLoadMoreGiftCard } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreGiftCard.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreGiftCard.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreGiftCard.failure(undefined));
  }
}

export function* watchLoadMoreGiftCard() {
  yield takeLatest(getActionType(actionLoadMoreGiftCard.request), handleGet);
}
