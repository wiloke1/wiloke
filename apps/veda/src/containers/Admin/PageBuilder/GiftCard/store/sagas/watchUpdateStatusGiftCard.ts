...
import { actionUpdateStatusGiftCard } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusGiftCard.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusGiftCard.success({
      ...
      yield put(actionUpdateStatusGiftCard.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusGiftCard.failure({ id: payload.id }));
...
export function* watchUpdateStatusGiftCard() {
  yield takeEvery(getActionType(actionUpdateStatusGiftCard.request), handle);
}
