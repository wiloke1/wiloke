...
import { actionDeleteGiftCard } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'giftCard', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteGiftCard.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteGiftCard.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteGiftCard.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteGiftCard: false }));
...
export function* watchDeleteGiftCard() {
  yield takeLatest(getActionType(actionDeleteGiftCard.request), handleDelete);
}
