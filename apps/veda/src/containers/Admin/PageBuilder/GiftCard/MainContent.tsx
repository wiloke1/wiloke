...
import { authSelector, giftCardSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import { useChangeSearchKey, useLoadMoreGiftCard, useSelectManyItems, useSetFilterType, useUpdateStatusGiftCard, useDeleteGiftCards } from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(giftCardSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusGiftCard();
  ...
  const loadMoreRequest = useLoadMoreGiftCard();
  const deletePageRequest = useDeleteGiftCards();
