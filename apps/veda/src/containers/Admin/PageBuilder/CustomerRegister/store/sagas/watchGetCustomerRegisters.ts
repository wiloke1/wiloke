...
import { actionGetCustomerRegisters } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionGetCustomerRegisters.request>) {
  ...
  try {
    ...
    yield put(actionGetCustomerRegisters.success({ data: response.info as BE_PageClient[] }));
  } catch (error) {
    yield put(actionGetCustomerRegisters.failure(undefined));
  }
}

export function* watchGetCustomerRegisters() {
  yield takeLatest(getActionType(actionGetCustomerRegisters.request), handleGet);
}
