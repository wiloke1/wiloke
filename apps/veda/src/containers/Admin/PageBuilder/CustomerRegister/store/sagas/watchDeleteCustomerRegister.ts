...
import { actionDeleteCustomerRegisters } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'register', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerRegisters.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerRegisters.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerRegisters.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersRegister: false }));
...
export function* watchDeleteCustomerRegister() {
  yield takeLatest(getActionType(actionDeleteCustomerRegisters.request), handleDelete);
}
