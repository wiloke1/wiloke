...
import { authSelector, customerRegisterSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerRegisters,
  useLoadMoreCustomerRegister,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerRegister,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerRegisterSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerRegister();
  ...
  const loadMoreRequest = useLoadMoreCustomerRegister();
  const deletePageRequest = useDeleteCustomerRegisters();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-register',
      entityVariant,
    });
