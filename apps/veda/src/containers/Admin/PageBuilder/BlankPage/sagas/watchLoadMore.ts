...
import { actionLoadMoreBlankPage } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreBlankPage.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreBlankPage.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreBlankPage.failure(undefined));
  }
}

export function* watchLoadMoreBlankPages() {
  yield takeLatest(getActionType(actionLoadMoreBlankPage.request), handleGet);
}
