...
import { actionUpdateStatusBlankPage } from '../actions';

function* handleUpdate({ payload }: ReturnType<typeof actionUpdateStatusBlankPage.request>) {
  ...
  try {
    ...
    const { statusSync }: UpdateStatusPublishResult = yield call(updateStatusPublish, {
      updateStatusPublishParams: handleUpdateStatusPage({
        eventId,
        data: adminPageData,
        isOverrideIndividualPages: true, // "Page" mặc định sẽ cho ghi đè
        isPublished: enable,
      }),
    });
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusBlankPage.success({
      ...
      yield put(actionUpdateStatusBlankPage.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusBlankPage.failure({ id: payload.id }));
...
export function* watchUpdateStatusBlankPage() {
  yield takeEvery(getActionType(actionUpdateStatusBlankPage.request), handleUpdate);
}
