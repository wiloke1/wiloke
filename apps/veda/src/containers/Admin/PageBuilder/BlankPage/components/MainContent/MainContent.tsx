import { notification } from 'antd';
import AsyncComponent from 'components/AsyncComponent';
import Empty from 'components/Empty';
import Box from 'components/FieldBox';
import SectionPageHeader from 'components/SectionPageHeader';
import { StatisticBox, StatisticBoxLoading } from 'components/StatisticCard';
import TextInput from 'components/TextInput';
import { useChangeModalAdminSettings } from 'containers/Admin/Modals';
import { useChangeSettingsDashboardPage } from 'containers/Admin/PageBuilder/DashboardPageSettings';
import { blankPageSelector } from 'containers/Admin/selector';
import { AdminPageData } from 'containers/Admin/types';
import withDebounce from 'hocs/withDebounce';
import { useSocketForSyncShopify } from 'hooks/useSocket/useSocketForSyncShopify';
import { FC, useCallback, useMemo } from 'react';
import { useSelector } from 'react-redux';
import { useHistory } from 'react-router-dom';
import { useSetGeneralSettingsPage } from 'store/global/generalSettings/slice';
import { authSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import { useChangeSearchKey, useDeleteBlankPages, useLoadMoreBlankPage, useSelectManyItems, useUpdateStatusBlankPage } from '../../actions';
import { useGetTemplatesPopup } from '../../actions/actionTemplate';
import { ButtonDeletePages } from '../SectionFilter/ButtonDeletePages';
import { SectionFilter } from '../SectionFilter/SectionFilter';
import { SelectType } from '../SectionFilter/SelectType';
import * as styles from './styles';

const DebounceInput = withDebounce(TextInput, 'value', 'onValueChange');

export const MainContent: FC = () => {
  const {
    data,
    updatePageStatus,
    ids,
    getAllPageStatus,
    isSelectAll,
    loadMorePageStatus,
    duplicateStatus,
    search,
    hasNextPage,
    filterType,
    deletePending,
  } = useSelector(blankPageSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusBlankPage();
  const selectIds = useSelectManyItems();
  const changeModalSettings = useChangeModalAdminSettings();
  const getTemplates = useGetTemplatesPopup();
  const history = useHistory();
  const setSettingPage = useSetGeneralSettingsPage();
  const changeSearchKey = useChangeSearchKey();
  const loadMoreRequest = useLoadMoreBlankPage();
  const changeSettings = useChangeSettingsDashboardPage();
  const deletePageRequest = useDeleteBlankPages();

  const { connect, disconnect, statusSocketConnection } = useSocketForSyncShopify();

  const _handlePublishPage = (item: AdminPageData) => (val: boolean) => {
    connect({
      onSuccess: () => {
        setPublished.request({
          id: item.commandId,
          enable: val,
          adminPageData: item,
          onFulfill: () => {
            disconnect({});
          },
        });
      },
      onError: () => {
        notification.error({ message: i18n.t('publish_shopify.init_sync_error') });
      },
    });
  };

  const _handleSelectPage = useCallback(
    (item: AdminPageData) => {
      if (!ids.includes(item.commandId)) {
        selectIds([...ids, item.commandId]);
      } else {
        selectIds(ids.filter(id => id !== item.commandId));
      }
    },
    [ids, selectIds],
  );

  const _handleGoToPage = (item: AdminPageData) => () => {
    const entityVariant = 'Client';
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      type: 'page',
      label: item.label,
      isCreate: false,
      backToPage: '/page/blank',
      entityVariant: 'Client',
    });
    setSettingPage({ settings: { label: item.label } });
  };

  const transformDate = (createTime?: number, updateTime?: number) => {
    return timeAgo(updateTime ?? createTime ?? 0, getDate(updateTime ?? createTime ?? Date.now()));
  };

  const TrackingLoadMore = useMemo(() => {
    const lastCursor = data.at(-1)?.commandId;
    if (hasNextPage && lastCursor) {
      return (
        <ViewportTracking
          offsetTop={-200}
          onEnterViewport={() => {
            if (loadMorePageStatus !== 'loading') {
              loadMoreRequest.request({
                s: search,
                pageType: 'page',
                filterType,
                lastCursor,
              });
            }
          }}
        >
          <StatisticBoxLoading />
          <StatisticBoxLoading />
          <StatisticBoxLoading />
        </ViewportTracking>
      );
    }
    return null;
  }, [data, filterType, hasNextPage, loadMorePageStatus, loadMoreRequest, search]);

  const _renderData = () => {
    return (
      <View>
        {data.map(item => (
          <StatisticBox
            key={item.commandId}
            selected={isSelectAll}
            enable={item.enable}
            label={item.label}
            image={item.image}
            date={transformDate(item.createdDateTimestamp, item.modifiedDateTimestamp)}
            loading={
              statusSocketConnection === 'loading' ||
              updatePageStatus[item.commandId] === 'loading' ||
              duplicateStatus[item.commandId] === 'loading' ||
              deletePending.length > 0
            }
            onGotoPage={_handleGoToPage(item)}
            onSelected={() => _handleSelectPage(item)}
            onActive={_handlePublishPage(item)}
            onEditSettings={() => {
              changeSettings({
                originPage: item,
                visible: true,
                page: item,
              });
            }}
            onDelete={() => {
              connect({
                onSuccess: () => {
                  deletePageRequest.request({ ids: [item.commandId], onFulfill: () => disconnect({}) });
                },
                onError: () => {
                  notification.error({
                    message: i18n.t('publish_shopify.init_sync_error'),
                  });
                },
              });
            }}
          />
        ))}
        {TrackingLoadMore}
      </View>
    );
  };

  const _handleSearch = (value: string) => {
    changeSearchKey(value);
  };

  return (
    <View>
      <SectionPageHeader
        title={i18n.t('general.prefix_page', { prefix: i18n.t('adminDashboard.blank') })}
        description={i18n.t('adminDashboard.page_description.regular')}
        onClick={() => {
          changeModalSettings({ createNormalPage: true });
          getTemplates.request({ type: 'page' });
        }}
      />

      <View row css={styles.sectionFilter}>
        <View columns={[12, 6, 6]}>
          <SelectType />
        </View>
        <View columns={[12, 4, 3]}>
          <View css={[styles.flex, { justifyContent: 'flex-end' }]}>
            <ButtonDeletePages />
            <DebounceInput
              sizeInput="medium"
              value={search}
              radius={6}
              block
              css={{ marginLeft: '8px', width: '260px' }}
              borderColor="gray3"
              placeholder={`${i18n.t('adminDashboard.search', { text: i18n.t('general.page') })}...`}
              onValueChange={_handleSearch}
            />
          </View>
        </View>
      </View>
      <Box radius={6} borderColor="gray2" css={{ padding: '10px' }}>
        <SectionFilter />
        <AsyncComponent
          status={getAllPageStatus}
          Request={
            <View>
              <StatisticBoxLoading />
              <StatisticBoxLoading />
              <StatisticBoxLoading />
            </View>
          }
          Success={_renderData()}
          Empty={
            <View css={{ padding: '20px 0' }}>
              <Empty text={i18n.t('adminDashboard.search_empty')} />
            </View>
          }
          isEmpty={getAllPageStatus === 'success' && data.length === 0}
        />
      </Box>
    </View>
  );
};

export default MainContent;
