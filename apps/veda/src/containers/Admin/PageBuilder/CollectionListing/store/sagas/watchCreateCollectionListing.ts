...
import { actionCreateCollectionListing } from '../actions';

function* handleCreate({ payload }: ReturnType<typeof actionCreateCollectionListing.request>) {
  const { templateId, name, includeHeaderFooter, callback } = payload;
  try {
    const { isCreate }: ModalAdminState = yield select(modalsSelector);

    if (isCreate) {
      yield put(actionCreateCollectionListing.success(undefined));
    ...
    yield put(actionCreateCollectionListing.failure(undefined));
  }
}

export function* watchCreateCollectionListing() {
  yield takeLatest(getActionType(actionCreateCollectionListing.request), handleCreate);
}
