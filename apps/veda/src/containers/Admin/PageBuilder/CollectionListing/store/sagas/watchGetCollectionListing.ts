...
import { actionGetCollectionListing } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionGetCollectionListing.request>) {
  ...
  try {
    ...
    yield put(actionGetCollectionListing.success({ data: response.info as BE_PageClient[] }));
  } catch (error) {
    yield put(actionGetCollectionListing.failure(undefined));
  }
}

export function* watchGetCollectionListing() {
  yield takeLatest(getActionType(actionGetCollectionListing.request), handleGet);
}
