...
import { actionLoadMoreCollectionListing } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreCollectionListing.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreCollectionListing.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreCollectionListing.failure(undefined));
  }
}

export function* watchLoadMoreCollectionListing() {
  yield takeLatest(getActionType(actionLoadMoreCollectionListing.request), handleGet);
}
