...
import { actionDeleteCollectionListing } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'collections', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCollectionListing.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCollectionListing.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCollectionListing.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCollectionListing: false }));
...
export function* watchDeleteCollectionListing() {
  yield takeLatest(getActionType(actionDeleteCollectionListing.request), handleDelete);
}
