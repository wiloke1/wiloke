...
import { actionUpdateStatusCollectionListing } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCollectionListing.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCollectionListing.success({
      ...
      yield put(actionUpdateStatusCollectionListing.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCollectionListing.failure({ id: payload.id }));
...
export function* watchUpdateStatusCollectionListing() {
  yield takeEvery(getActionType(actionUpdateStatusCollectionListing.request), handle);
}
