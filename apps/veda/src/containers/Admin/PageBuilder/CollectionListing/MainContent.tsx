...
import { authSelector, collectionListingSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCollectionListings,
  useLoadMoreCollectionListing,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCollectionListing,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(collectionListingSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCollectionListing();
  ...
  const loadMoreRequest = useLoadMoreCollectionListing();
  const deletePageRequest = useDeleteCollectionListings();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/collection-listing',
