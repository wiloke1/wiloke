...
import { actionGetCustomerAccounts } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionGetCustomerAccounts.request>) {
  ...
  try {
    ...
    yield put(actionGetCustomerAccounts.success({ data: response.info as BE_PageClient[] }));
  } catch (error) {
    yield put(actionGetCustomerAccounts.failure(undefined));
  }
}

export function* watchGetCustomerAccounts() {
  yield takeLatest(getActionType(actionGetCustomerAccounts.request), handleGet);
}
