...
import { actionUpdateStatusCustomerAccount } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCustomerAccount.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCustomerAccount.success({
      ...
      yield put(actionUpdateStatusCustomerAccount.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCustomerAccount.failure({ id: payload.id }));
...
export function* watchUpdateStatusCustomerAccount() {
  yield takeEvery(getActionType(actionUpdateStatusCustomerAccount.request), handle);
}
