...
import { actionDeleteCustomerAccounts } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'account', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerAccounts.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerAccounts.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerAccounts.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersAccount: false }));
...
export function* watchDeleteCustomerAccount() {
  yield takeLatest(getActionType(actionDeleteCustomerAccounts.request), handleDelete);
}
