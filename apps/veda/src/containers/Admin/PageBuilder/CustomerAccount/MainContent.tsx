...
import { authSelector, customerAccountSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerAccounts,
  useLoadMoreCustomerAccount,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerAccount,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerAccountSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerAccount();
  ...
  const loadMoreRequest = useLoadMoreCustomerAccount();
  const deletePageRequest = useDeleteCustomerAccounts();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-account',
      entityVariant,
    });
