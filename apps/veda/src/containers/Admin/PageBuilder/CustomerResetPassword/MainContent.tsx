...
import { authSelector, customerResetPasswordSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerResetPasswords,
  useLoadMoreCustomerResetPassword,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerResetPassword,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerResetPasswordSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerResetPassword();
  ...
  const loadMoreRequest = useLoadMoreCustomerResetPassword();
  const deletePageRequest = useDeleteCustomerResetPasswords();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-reset-password',
      entityVariant,
    });
