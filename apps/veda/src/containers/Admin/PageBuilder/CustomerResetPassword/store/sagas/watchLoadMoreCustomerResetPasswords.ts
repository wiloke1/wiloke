...
import { actionLoadMoreCustomerResetPassword } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreCustomerResetPassword.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreCustomerResetPassword.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreCustomerResetPassword.failure(undefined));
  }
}

export function* watchLoadMoreCustomerResetPasswords() {
  yield takeLatest(getActionType(actionLoadMoreCustomerResetPassword.request), handleGet);
}
