...
import { actionGetCustomerResetPasswords } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionGetCustomerResetPasswords.request>) {
  ...
  try {
    ...
    yield put(actionGetCustomerResetPasswords.success({ data: response.info as BE_PageClient[] }));
  } catch (error) {
    yield put(actionGetCustomerResetPasswords.failure(undefined));
  }
}

export function* watchGetCustomerResetPasswords() {
  yield takeLatest(getActionType(actionGetCustomerResetPasswords.request), handleGet);
}
