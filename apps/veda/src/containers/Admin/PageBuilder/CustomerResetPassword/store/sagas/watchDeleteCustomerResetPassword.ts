...
import { actionDeleteCustomerResetPasswords } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'resetPassword', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerResetPasswords.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerResetPasswords.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerResetPasswords.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersResetPassword: false }));
...
export function* watchDeleteCustomerResetPassword() {
  yield takeLatest(getActionType(actionDeleteCustomerResetPasswords.request), handleDelete);
}
