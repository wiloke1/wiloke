...
import { actionUpdateStatusCustomerResetPassword } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCustomerResetPassword.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCustomerResetPassword.success({
      ...
      yield put(actionUpdateStatusCustomerResetPassword.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCustomerResetPassword.failure({ id: payload.id }));
...
export function* watchUpdateStatusCustomerResetPassword() {
  yield takeEvery(getActionType(actionUpdateStatusCustomerResetPassword.request), handle);
}
