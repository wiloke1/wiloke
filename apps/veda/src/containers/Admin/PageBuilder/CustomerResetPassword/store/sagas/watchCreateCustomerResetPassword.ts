...
import { actionCreateCustomerResetPassword } from '../actions';

function* handleCreate({ payload }: ReturnType<typeof actionCreateCustomerResetPassword.request>) {
  const { templateId, name, includeHeaderFooter, callback } = payload;
  try {
    const { isCreate }: ModalAdminState = yield select(modalsSelector);

    if (isCreate) {
      yield put(actionCreateCustomerResetPassword.success(undefined));
    ...
    yield put(actionCreateCustomerResetPassword.failure(undefined));
  }
}

export function* watchCreateCustomerResetPassword() {
  yield takeLatest(getActionType(actionCreateCustomerResetPassword.request), handleCreate);
}
