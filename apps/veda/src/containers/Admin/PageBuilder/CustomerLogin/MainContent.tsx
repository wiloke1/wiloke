...
import { authSelector, customerLoginSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerLogins,
  useLoadMoreCustomerLogin,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerLogin,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerLoginSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerLogin();
  ...
  const loadMoreRequest = useLoadMoreCustomerLogin();
  const deletePageRequest = useDeleteCustomerLogins();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-login',
