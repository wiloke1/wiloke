...
import { actionUpdateStatusCustomerLogin } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCustomerLogin.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCustomerLogin.success({
      ...
      yield put(actionUpdateStatusCustomerLogin.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCustomerLogin.failure({ id: payload.id }));
...
export function* watchUpdateStatusCustomerLogin() {
  yield takeEvery(getActionType(actionUpdateStatusCustomerLogin.request), handle);
}
