...
import { actionLoadMoreCustomerLogin } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreCustomerLogin.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreCustomerLogin.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreCustomerLogin.failure(undefined));
  }
}

export function* watchLoadMoreCustomerLogins() {
  yield takeLatest(getActionType(actionLoadMoreCustomerLogin.request), handleGet);
}
