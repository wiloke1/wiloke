...
import { actionDeleteCustomerLogins } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'login', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerLogins.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerLogins.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerLogins.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersLogin: false }));
...
export function* watchDeleteCustomerLogin() {
  yield takeLatest(getActionType(actionDeleteCustomerLogins.request), handleDelete);
}
