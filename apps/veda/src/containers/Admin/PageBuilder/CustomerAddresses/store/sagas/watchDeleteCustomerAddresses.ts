...
import { actionDeleteCustomerAddresses } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'addresses', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerAddresses.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerAddresses.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerAddresses.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersAddresses: false }));
...
export function* watchDeleteCustomerAddresses() {
  yield takeLatest(getActionType(actionDeleteCustomerAddresses.request), handleDelete);
}
