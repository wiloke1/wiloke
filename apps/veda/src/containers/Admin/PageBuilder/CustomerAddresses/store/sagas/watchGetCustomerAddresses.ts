...
import { actionGetCustomerAddresses } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionGetCustomerAddresses.request>) {
  ...
  try {
    ...
    yield put(actionGetCustomerAddresses.success({ data: response.info as BE_PageClient[] }));
  } catch (error) {
    yield put(actionGetCustomerAddresses.failure(undefined));
  }
}

export function* watchGetCustomerAddresses() {
  yield takeLatest(getActionType(actionGetCustomerAddresses.request), handleGet);
}
