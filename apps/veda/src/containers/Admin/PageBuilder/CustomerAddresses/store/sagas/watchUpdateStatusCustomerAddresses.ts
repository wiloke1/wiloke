...
import { actionUpdateStatusCustomerAddresses } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCustomerAddresses.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCustomerAddresses.success({
      ...
      yield put(actionUpdateStatusCustomerAddresses.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCustomerAddresses.failure({ id: payload.id }));
...
export function* watchUpdateStatusCustomerAddresses() {
  yield takeEvery(getActionType(actionUpdateStatusCustomerAddresses.request), handle);
}
