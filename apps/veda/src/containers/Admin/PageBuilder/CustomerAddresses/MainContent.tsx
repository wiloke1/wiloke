...
import { authSelector, customerAddressesSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerAddresses,
  useLoadMoreCustomerAddresses,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerAddresses,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerAddressesSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerAddresses();
  ...
  const loadMoreRequest = useLoadMoreCustomerAddresses();
  const deletePageRequest = useDeleteCustomerAddresses();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-addresses',
