...
import { actionDeleteCustomerOrders } from '../actions';
...
function* handleDeleteOnePage(id: string) {
  ...
  const { statusDeletePage }: DeletePageResult = yield call(deletePage, {
    deletePageParams: handleDeletePageInDashboard({ pageCommandId: id, pageType: 'order', eventId }),
  });
  ...
  if (statusDeletePage === 'success') {
    ...
    // Rate limit shopify
    yield delay(500);
    yield put(actionDeleteCustomerOrders.success({ ids: [id] }));
    notifyAxiosHandler.handleSuccess(response.message);
  } else {
    // @tuong -> Retry hoặc lưu nhật kí để thực hiện lại, ...
    yield put(actionDeleteCustomerOrders.failure({ id }));
  }
}

function* handleDelete({ payload }: ReturnType<typeof actionDeleteCustomerOrders.request>) {
  ...
  try {
    ...
    yield put(changeModalAdminSettings({ deleteCustomersOrder: false }));
...
export function* watchDeleteCustomerOrder() {
  yield takeLatest(getActionType(actionDeleteCustomerOrders.request), handleDelete);
}
