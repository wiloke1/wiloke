...
import { actionLoadMoreCustomerOrder } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreCustomerOrder.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreCustomerOrder.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreCustomerOrder.failure(undefined));
  }
}

export function* watchLoadMoreCustomerOrders() {
  yield takeLatest(getActionType(actionLoadMoreCustomerOrder.request), handleGet);
}
