...
import { actionUpdateStatusCustomerOrder } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusCustomerOrder.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusCustomerOrder.success({
      ...
      yield put(actionUpdateStatusCustomerOrder.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusCustomerOrder.failure({ id: payload.id }));
...
export function* watchUpdateStatusCustomerOrder() {
  yield takeEvery(getActionType(actionUpdateStatusCustomerOrder.request), handle);
}
