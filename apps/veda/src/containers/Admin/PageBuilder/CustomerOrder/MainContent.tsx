...
import { authSelector, customerOrderSelector } from 'store/selectors';
import { i18n } from 'translation';
import { getDate, timeAgo } from 'utils/timeAgo';
import { View, ViewportTracking } from 'wiloke-react-core';
import {
  useChangeSearchKey,
  useDeleteCustomerOrders,
  useLoadMoreCustomerOrder,
  useSelectManyItems,
  useSetFilterType,
  useUpdateStatusCustomerOrder,
} from '.';
...
export const MainContent = () => {
  const {
    ...
    hasNextPage,
    deletePending,
  } = useSelector(customerOrderSelector);
  const { shopName } = useSelector(authSelector);

  const setPublished = useUpdateStatusCustomerOrder();
  ...
  const loadMoreRequest = useLoadMoreCustomerOrder();
  const deletePageRequest = useDeleteCustomerOrders();
  ...
  const _handleGoToPage = (item: AdminPageData) => () => {
    ...
    history.push(`/builder?shop=${shopName}&id=${item.commandId}&entityVariant=${entityVariant}`, {
      ...
      label: item.label,
      isCreate: false,
      backToPage: '/page/customer-order',
