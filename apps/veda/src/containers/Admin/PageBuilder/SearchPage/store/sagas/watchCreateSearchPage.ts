...
import { actionCreateSearchPage } from '../actions';

function* handleCreate({ payload }: ReturnType<typeof actionCreateSearchPage.request>) {
  const { templateId, name, includeHeaderFooter, callback } = payload;
  try {
    const { isCreate }: ModalAdminState = yield select(modalsSelector);

    if (isCreate) {
      yield put(actionCreateSearchPage.success(undefined));
    ...
    yield put(actionCreateSearchPage.failure(undefined));
  }
}

export function* watchCreateSearchPage() {
  yield takeLatest(getActionType(actionCreateSearchPage.request), handleCreate);
}
