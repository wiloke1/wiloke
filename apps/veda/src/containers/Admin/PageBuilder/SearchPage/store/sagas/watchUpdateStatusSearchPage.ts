...
import { actionUpdateStatusSearchPage } from '../actions';

function* handle({ payload }: ReturnType<typeof actionUpdateStatusSearchPage.request>) {
  ...
  try {
    ...
    if (statusSync === 'success') {
      ...
      yield put(
        actionUpdateStatusSearchPage.success({
      ...
      yield put(actionUpdateStatusSearchPage.failure({ id }));
    }
  } catch (error) {
    yield put(actionUpdateStatusSearchPage.failure({ id: payload.id }));
...
export function* watchUpdateStatusSearchPage() {
  yield takeEvery(getActionType(actionUpdateStatusSearchPage.request), handle);
}
