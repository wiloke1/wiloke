...
import { actionLoadMoreCollectionPage } from '../actions';

function* handleGet({ payload }: ReturnType<typeof actionLoadMoreCollectionPage.request>) {
  ...
  try {
    ...
    yield put(actionLoadMoreCollectionPage.success({ data: responseData, hasNextPage: responseData.length > 0 ? true : false }));
  } catch (error) {
    yield put(actionLoadMoreCollectionPage.failure(undefined));
  }
}

export function* watchLoadMoreCollectionPages() {
  yield takeLatest(getActionType(actionLoadMoreCollectionPage.request), handleGet);
}
