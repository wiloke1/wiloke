...
declare type CheckElementExistInArray<T, X> = T extends readonly [X, ...infer _Rest] ? true : T extends readonly [X] ? true : T extends readonly [infer _, ...infer Rest] ? CheckElementExistInArray<Rest, X> : false;
...
declare type UniqueArray<T> = T extends readonly [infer X, ...infer Rest] ? (CheckElementExistInArray<Rest, X> extends true ? ['Encountered value with duplicates:', X] : readonly [X, ...UniqueArray<Rest>]) : T;

declare type FlatArray<A extends unknown[]> = ReturnType<A extends [infer U, ...infer V] ? (U extends unknown[] ? () => [...U, ...FlatArray<V>] : () => [U, ...FlatArray<V>]) : () => A>;
...
declare type DeepOmit<T, Filter extends DeepModify<T>> = T extends IsBuiltIn
	...
	? {
			[K in keyof T as K extends keyof Filter ? (Filter[K] extends true ? never : K) : K]: K extends keyof Filter ? (Filter[K] extends DeepModify<T[K]> ? DeepOmit<T[K], Filter[K]> : T[K]) : T[K];
	  }
	: never;
...
declare type Zip_DeepMergeTwoTypes<T, U> = T extends [] ? U : U extends [] ? T : [DeepMergeTwoTypes<Head<T>, Head<U>>, ...Zip_DeepMergeTwoTypes<Tail<T>, Tail<U>>];
/**
...
declare type PathKeys<T> = object extends T ? string : T extends readonly any[] ? Extract<keyof T, `${number}`> | SubKeys<T, Extract<keyof T, `${number}`>> : T extends object ? Extract<keyof T, string> | SubKeys<T, Extract<keyof T, string>> : never;
declare type PathKeyOfObject<T, Key extends keyof T = keyof T> = Key extends string ? (T[Key] extends Record<string, any> ? `${Key}.${PathKeyOfObject<T[Key], Exclude<keyof T[Key], keyof Array<any>>> & string}` | `${Key}.${Exclude<keyof T[Key], keyof Array<any>> & string}` | Key : Key) : never;

declare type PathValue<T, P extends PathKeyOfObject<T>> = P extends `${infer Key}.${infer Rest}` ? (Key extends keyof T ? (Rest extends PathKeyOfObject<T[Key]> ? PathValue<T[Key], Rest> : never) : never) : P extends keyof T ? T[P] : never;
...
declare type Replace<Source extends string, SearchValue extends string, NewValue extends string> = SearchValue extends '' ? Source : Source extends `${infer Head}${SearchValue}${infer Tail}` ? `${Head}${NewValue}${Tail}` : Source;
...
declare type KebabCase<T> = T extends `${infer Character}${infer Rest}` ? (Rest extends Uncapitalize<Rest> ? `${Uncapitalize<Character>}${KebabCase<Rest>}` : `${Uncapitalize<Character>}-${KebabCase<Rest>}`) : T;
...
export type ToPartialKeys<T, RK extends keyof T> = Omit<T, RK> & Partial<Pick<T, RK>>;
// export type ToRequiredKeys<T, RK extends keyof GetOptional<T>> = Omit<T, RK> & Required<Pick<T, RK>>;
