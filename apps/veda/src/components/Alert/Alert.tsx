...
import { FontAwesome, FontAwesomeName, Text, View, ViewProps } from 'wiloke-react-core';
...
const Alert = forwardRef<HTMLElement, AlertProps>(
  ({ closable = true, description, message, showIcon = true, type = 'info', size = 'medium', Icon, onClose, borderWidth = 1, ...rest }, ref) => {
    const iconNameMapping: Record<AlertType, FontAwesomeName> = {
    ...
    const renderAlertIcon = () => {
      ...
      if (Icon) {
        return <View css={css.icon(size)}>{Icon}</View>;
      }
      return (
        <View css={css.icon(size)}>
          <FontAwesome type="far" color={type} name={iconNameMapping[type]} />
        </View>
      );
    };
