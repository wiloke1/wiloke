    ...
    <>
      {variant === 'default' ? (
        <Text tagName="label" css={css.container(disabled, size)}>
          ...
          {children && (
            <Text onClick={onClick} css={{ padding: `0 8px`, display: 'inline-block', verticalAlign: 'middle' }} tagName="span">
              {children}
            </Text>
          )}
        </Text>
        ...
        >
          <Text tagName="span" onClick={onClick} css={css.radioButtonWrapper}>
            {_renderRadioNative()}
          </Text>
          {children}
        </Text>
