...
import { addMegaMenusToPage, getPage, setThemeAddonsToPages, setThemeHeaderFooterToPages } from 'store/actions/actionPages';
...
export function* handleGetPageOfThemeDraft_({ payload }: ReturnType<typeof getPage.request>) {
  const { id } = payload;
  try {
    ...
    /** Xử lý "pageSettings" */
    const page: Awaited<ReturnType<typeof getPageOfThemeDraft>> = yield retry(3, 1000, getPageOfThemeDraft, { commandId: id });
    ...
    const sections: Awaited<ReturnType<typeof getSectionsOfPageInThemeDraft>> = yield retry(3, 1000, getSectionsOfPageInThemeDraft, {
      sectionCommandIds,
    });
    ...
    // sau khi getPage/success thì thêm addons và footer vào pages
