...
export function* updateShopifyPages({ updateShopifyPagesParams }: UpdateShopifyPages) {
  ...
  if (unpublishAllParameters) {
    yield retry(3, 1000, shopifyConnectionService.writePageToShopify, { ...unpublishAllParameters, isPreview: true });
  ...
  yield retry(3, 1000, shopifyConnectionService.writePageToShopify, { ...updateShopifyPagesParameters, isPreview: true });
