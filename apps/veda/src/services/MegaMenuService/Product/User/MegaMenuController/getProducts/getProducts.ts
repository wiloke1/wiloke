...
import { transformSections } from 'services/ResultService/ultils/transformSections';
...
export const getProducts = async ({ type, ...params }: GetProducts | LoadmoreProducts) => {
  ...
  const transformedSections = transformSections(response.data.info);

  return {
    info: transformedSections as ProductSection[],
