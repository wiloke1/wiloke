...
export const getCategoriesOfDraft = async () => {
  ...
  if (role === 'dev') {
    const response = await sectionApiController.atom.devApi.category.getCategoriesOfDraft({ type: 'GET FIRST PAGE' });
...
export const loadMoreCategoriesOfDraft = async (cursor: string) => {
  const { role } = getUserInfo();
  if (role === 'dev') {
    const response = await sectionApiController.atom.devApi.category.getCategoriesOfDraft({ type: 'LOADMORE', cursor });
    return adapterGetCategories(response.info);
  }
  if (role === 'admin') {
    const response = await sectionApiController.atom.devApi.category.getCategoriesOfDraft({ type: 'LOADMORE', cursor });
