...
export interface I18n<T extends Record<string, any>> {
  ...
  getLocale: () => string;
  t<K extends Path<T>>(key: K, options?: OptionKeys<PathValue<T, K>> extends never ? { textTransform?: TextTransform } : { [P in OptionKeys<PathValue<T, K>>]: string } & { textTransform?: TextTransform }): string;
}
