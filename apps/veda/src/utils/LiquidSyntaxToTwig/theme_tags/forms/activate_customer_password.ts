 ...
 */
export const activate_customer_password = (liquid: string) => {
  return liquid.replace(/{%\s*form.*activate_customer_password.*%}/gm, BOC => {
    const _BOC = handlePreprocess(BOC);
    return _BOC.replace(/{%\s*form.*activate_customer_password.*%}/, value => {
      const form = handleFormAttributes({
        onException: err => {
          return new LiquidSyntaxToTwigError(i18n.t('twig_error.theme_tags.forms.activate_customer_password', { error_signal: toString(err) }));
        },
